...
associations$terms <- factor(x = associations$terms, # turns the new column unequivocally into
                             levels = associations$terms)  # factors
associations <- associations[order(associations$terms,decreasing = T), ] # decreasing association 
```

Now let's build the visualization Kwatler shows, here with reproducible code:
...
The most correlated words with our target *apologies* are *delay* and *issues*. Also *latearriving*  appears among the third most associated words. The word *refund* appears among the 5 words closest to the 
threashold value set to find associations. One may infer based on these findings that there is evidence 
that Delta customer service agents do apologize for issues causing delays in arrivals and may offer refunds 
to folow up the apology.
These findings trigger more questions about how often these events happen and what consequences bring to the
image and the bottom line of the company.
