...
Text mining among this curated sample of Tweets for the month of October 
from the Delta Airlines Customer Service team 
looks to answer a few relevant questions for other similar teams.
...
3. How many team members are needed for a similar size of operation?
4. How many social media responses per customer service representative are reasonable?

Some of these can be addressed without the full data mining preprocessing, actually
the dates are critical for questions 1 and 4 so they are better answered before doing the 
preparation and cleaning steps.

For the sake of completeness though, those steps are shown below.
...
For this Ted Kwartler suggests on page 32 of his [book][ref2], using a function to detect the last two characters of every tweet, hoping to capture the initials used for each agent.
Here the last word of the whole tweet is extracted using `stri_extract_all_boundaries` from the `stringi` package.
This hopefully covers the case of variable number of letters or non-standard initials. 

```{r responses per agent}
initials <- sapply(X = stri_extract_all_boundaries(clean.dataframe[,1]), FUN = function(vs) {lvs <- length(vs); vs[lvs] } )
table(initials)
```
