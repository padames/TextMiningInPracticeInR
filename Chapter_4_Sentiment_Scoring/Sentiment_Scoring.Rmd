...
The workflow involves converting the UTF-8 encoding of the emois to **R byte enconding**, then
transalate the R-byte into plain English text to be used in sentiment analysis. This is accomplished in the following block of R code.

```{r converting emoji current text representation in UTF-8 block encoding to standard UTF-8}
bos.airbnb$comments_utf8 <- iconv(bos.airbnb$comments_original, from = "ascii", to = "utf8", sub = "byte")
```
...
```{r punctuation-based emoticons}
bos.airbnb$comments_enhanced2 <- mgsub(pattern = emoticon[,2], 
                                       replacement = emoticon[,1], 
                                       bos.airbnb$comments_ascii)
```
